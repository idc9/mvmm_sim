...
def run_command(py_script, py_params, bayes_params, args,
                break_first=False):
    """
    ...
    if which_computer() == 'iain_laptop' or args.no_submit:
        bayes_args_str = ''
    else:
        bayes_args_str = arg_str(bayes_params)

    # py_script = os.path.join(Paths().script_dir, py_script)
    command = 'python {} {} {}'.format(py_script,
                                       py_args_str,
                                       bayes_args_str)
    print(command)
...
def submit_expers(py_script, py_params, bayes_params, args,
    ...
    for i, py_args in enumerate(py_args_list):
        run_command(py_script=py_script,
                    py_params=py_args,
                    bayes_params=bayes_params,
                    args=args)

    save_params(py_args_list=py_args_list,
                data_dir=data_dir,
                base_name=base_name)
